...
void quicksort_1p_r (int *arr, int left, int right, stats_t *find) {
	...
	if (right > left) {	
		...
		quicksort_1p_r(arr, left, i-1, find);

		quicksort_1p_r(arr, i, right, find);
	}
}
...
int main (int argc, char *argv[]) {
	...
	start = clock();
	quicksort_1p_r(arr.table, 0, arr.inserted-1, &stats);
	end = clock();
